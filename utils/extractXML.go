...
func ExtractAssets(dsCSV models.DatasourceStruct) (models.DatasourceStruct, error) {

	file, err := os.Open("storage/download.tds")
	if err != nil {
		return dsCSV, err
	}
	defer file.Close()
	...
	for _, table := range dsExists.Connection.Relations {
		...
		tableID, columnIDs, err := lib.GetColumnIDs(dsCSV.Database, strings.Trim(table.Name, "[]"))
		if err != nil {
			errorMsg = err
		}
					...
					if strings.Trim(col.ParentName, "[]") == strings.Trim(table.Name, "[]") {
						...
						if !containsColumn(strings.Trim(col.RemoteName, "[]"), table.Name, dsCSV.Tables) {
							...
							t.Columns = append(t.Columns, models.ColumnStruct{
								ColumnName:        col.RemoteName,
								ColumnType:        col.LocalType,
								ColumnDescription: "",
								DataElements:      label, // write func to get label on asset
							})
						}
		...
		if !found {
			...
			dsCSV.Tables = append(dsCSV.Tables, models.TableStruct{
				...
				TableType:       "",
				ContentProfiles: label, // write func to get label on asset
				Columns:         columns,
			})
		}
