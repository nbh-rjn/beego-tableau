...
func (c *TableauController) PostSync() {
	...
	c.Logger.Info("sync body parsed successfully")
	...
	c.Logger.Info("csv parsed successfully")

	// add more tables, cols (assets)
	if requestBody.CreateNewAssets {
		c.Logger.Info("creating new assets")
		...
		c.Logger.Info("current data sources fetched successfully")
		...
		if datasourceID, exists := utils.DatasourceExists(currentDatasources, datasourceRecord.Datasource); exists {
			c.Logger.Info("data source already exists")
			...
			c.Logger.Info("tds file of existing data source downloaded successfully")
			...
			c.Logger.Info("assets extracted from current data source successfully")
		...
		c.Logger.Info("tds file of all assets generated successfully")
		...
		c.Logger.Info("data source file published successfully")
		c.Logger.Info("may take time for changes to update on Tableau")

	}

	if requestBody.LabelAssets {
		c.Logger.Info("labelling assets")
		...
		if err := CallWithRetry(c.Ctx.Request.Context(), call); err != nil {
			c.Logger.Info("category already exists")
		}
		...
		for i := 1; i <= len(datasourceRecord.Tables); i++ {
			errorMsg = <-results
			if errorMsg != nil {
				c.Logger.Error("error labelling", "err", errorMsg)
			}
		}
